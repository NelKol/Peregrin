...
def server(input: Inputs, output: Outputs, session: Session):
                ...
                panels.append(
                    ui.accordion_panel(
                        ...
                        ui.panel_well(
                            ...
                            # ui.output_ui(f"threshold2d_widget_placeholder_{threshold_id}"),
                            ui.div(
                            ...
                            ui.markdown("<p style='font-size:1px; line-height:0.1; color:#f7f7f7;'>_</p>"),
                            ui.input_action_button(id=f"pass_selected{threshold_id}", label="Pass Selected", class_="space-x-2", onclick=f"Shiny.setInputValue('pass_selected', '{threshold_id}', {{priority: 'event'}});"),
                        ),
                    ),
                ),
            panels.append(
                ui.accordion_panel(
                    "Filter settings",
                    ui.input_numeric("threshold2d_array_size", "Dot Size", value=3, min=0, step=1),
                    ui.markdown("<p style='font-size:3px; line-height:0.8; color:white;'>_</p>"),
                    ui.input_action_button("threshold_dimensional_toggle", dimension_button_label.get(), width="100%"),
                ),
            ),
    ...
    #     selected_id = input.pass_selected()
    #     # print(f"Pass selected: {selected_id}")
    #     render_threshold2d_widget(selected_id + 1)
